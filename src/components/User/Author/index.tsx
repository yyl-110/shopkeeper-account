...
export default ({
  ...
  const renderDetail = () => {
    return (
      <Space size="small" className={classnames('user-author', `user-author-${size}`, sizeClass, className)}>
        {(!model || model === 'avatar') && (
          <div className="user-author-avatar">
            <UserBrand id={id}>
              <Avatar shape={avatarShape} size={avatarSize} src={avatar || settings.defaultAvatar} />
            </UserBrand>
          </div>
        )}
        {(!model || model === 'name') && (
          <div className="user-author-content">
            ...
            {description && <p className="user-author-desc" dangerouslySetInnerHTML={{ __html: description }} />}
          </div>
        )}
