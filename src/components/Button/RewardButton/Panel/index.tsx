...
export default ({ visible, onVisibleChange, author, amount, type, doPay }) => {
        ...
        <Row gutter={[24, 24]} className={styles['reward-selector']}>
          {pays.map(({ key, title }) => (
            <Col key={key} span={24 / pays.length}>
              <Button onClick={() => setPayType(key)} className={classNames({ [styles.active]: payType === key })}>
                {title}
              </Button>
            </Col>
          ))}
  ...
  return (
    <>
      <Modal title={null} footer={null} open={visible} destroyOnClose onCancel={() => onVisibleChange(false)}>
