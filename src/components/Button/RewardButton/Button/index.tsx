...
export default ({ author, dataType, dataKey, dataSource, text, children, ...props }) => {
    ...
    return (
      <>
        <Space size="small" className={styles['reward-list']}>
          ...
          {data.length > 8 && (
            <Button shape="circle" size="small" icon={<EllipsisOutlined />} onClick={() => setMoreVisible(true)} />
          )}
  ...
  return (
    <>
      ...
      />
      <Modal title="打赏列表" footer={null} open={moreVisible} destroyOnClose onCancel={() => setMoreVisible(false)}>
