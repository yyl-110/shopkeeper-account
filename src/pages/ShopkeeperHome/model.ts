  ...
  reducers: {
    setRecommends(state, { payload }) {
      return setList('recommends', payload, state, (child, item) => {
        if (child.type === 'article' && item.type === 'article') return child.object.id === item.object.id;
        if (child.type === 'question' && item.type === 'question') {
          return child.object.answer_list.find(answer => {
            return item.object.answer_list.find(itemAnswer => answer.id === itemAnswer.id);
          });
        }
      });
    },
    ...
    replaceRecommendsAnswer(state, { payload }) {
      return replaceItem(
        ...
        state,
        item => {
          return item.type === 'question' && item.object.answer_list.find(answer => answer.id === payload.id);
        },
        item => {
