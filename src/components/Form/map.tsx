      ...
      content: value => {
        ...
        const getMessage = (error, message) => {
          ...
          return (
            <p>
              <span className="">
                {error ? <InfoCircleOutlined className={className} /> : <CheckCircleOutlined className={className} />}
              </span>
              {message}
            </p>
          );
        ...
        return (
          <div className={styles['help-body']}>
            ...
            {getMessage(lengthError, `长度为${PASSWORD_LENGTH_MIN}-${PASSWORD_LENGTH_MAX}个字符`)}
