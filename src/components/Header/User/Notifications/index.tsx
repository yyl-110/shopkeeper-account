...
import { BellFilled, BarsOutlined, HeartFilled, UserSwitchOutlined, SettingFilled } from '@ant-design/icons';
...
export default ({ overflowCount }) => {
  ...
  useEffect(() => {
    const connection = () => {
      ...
      if (!token) return;
      const webSocket = new ReconnectingWebSocket(ws + '?token=' + encodeURIComponent(token), [], {
  ...
  const content = (
    <div className={styles['notifications']}>
      <Tabs className={styles['tabs']} activeKey={activeKey} onChange={activeKey => setActiveKey(activeKey)}>
  ...
  return (
    ...
    >
      <Badge count={groupCount.count} overflowCount={overflowCount} className={styles['badge']}>
        <BellFilled />
      </Badge>
    </Popover>
  );
